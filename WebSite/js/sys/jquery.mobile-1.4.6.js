    ...
    (function ($, window, document, undefined) {
        ...
        function triggerVirtualEvent(eventType, e, flags) {
            ...
            function triggerEvent(direction) {
                if (direction) {
                    $app.touch.pageTransitionCallback = triggerEvent;
            ...
            if ((flags && flags[eventType]) || (!flags && getClosestElementWithVirtualBinding(target, eventType))) {
                ve = createVirtualEvent(e, eventType);
                target = $(target);
                if (eventType === 'vclick') {
                    screen = $app.touch.screen();
                    if (screen.splitTop || screen.splitLeft) {
                        clickTarget = $(document.elementFromPoint(e.clientX, e.clientY));
                        readingPageDetailIsActive = $.mobile.activePage.is('.app-reading-pane-detail');
                        detailPane = clickTarget.closest('.app-reading-pane-detail');
                        if (readingPageDetailIsActive && !detailPane.length && clickTarget.closest('.app-reading-pane-master,.app-sidebar,.app-bar-toolbar').length)
                            pageChangeDirection = -1;
                        else if (!readingPageDetailIsActive && clickTarget.closest('.app-reading-pane-detail').length)
                            pageChangeDirection = 1;
                    }
                }

                if (pageChangeDirection)
                    setTimeout(triggerEvent, 0, pageChangeDirection);
                else
                    triggerEvent();
            }
